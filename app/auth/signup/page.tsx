    ...
    const signupUser = async(event: React.SyntheticEvent<HTMLFormElement>) => {
        ...
        const { data, error } = await authClient.signUp.email({
            ...
            onError: (ctx) => {
                ...
                if ((errorCode as string).toString().toLowerCase().includes("email")) {
                    setEmailHelperText(ctx.error.message);
                } else if ((errorCode as string).toString().toLowerCase().includes("password")) {
                    setEmailHelperText(ctx.error.message);
                }
            }
        })
                    ...
                    <div>
                        <div className="flex flex-col items-center justify-center gap-3 p-10">
                            <div className="flex flex-col items-center justify-center">
                                ...
                                <h1 className="font-black text-xl mt-[-10px]">Sign up</h1>
                            </div>
                            <div>
                            ...
                            <div>
                                <h6 className="font-semibold text-xs text-slate-300">Name</h6>
                                <input placeholder="Enter your name" name="name" className={`outline p-2 rounded-sm text-slate-100 ${ nameHelperText ? "outline-red-500" : "outline-slate-700" }`}></input>
                                <span className="w-full">{
                                    nameHelperText && <h6 className="font-semibold text-xs text-red-500">{nameHelperText}</h6>
                                }</span>
                            </div>
                            <div>
                            ...
                            <span className="w-full">
                                <h6 className="font-semibold text-xs text-blue-500">Already have an account? <a className="text-blue-300 hover:text-blue-400" href="/auth/signin">Sign in!</a></h6>
                            </span>
